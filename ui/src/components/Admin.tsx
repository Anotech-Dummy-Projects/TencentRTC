  ...
  import { getAdminMe, getAdminSessionMessages, getAdminSessionTimeline, getAdminSessions } from "../lib/api";
  ...
  export const Admin: React.FC<AdminProps> = ({ onLogout, onNavigateHome, onUnauthorized }) => {
    ...
    useEffect(() => {
      ...
      getAdminSessionMessages(selectedSessionId)
        .then(({ archived, messages }) => {
          if (cancelled) return;
          setSessions((current) => current.map((session) =>
            session.id === selectedSessionId
              ? { ...session, messages: archived ? messages : [] }
              : session
          ));
        })
        .catch((error: any) => {
          if (!cancelled) setMessageError(error.response?.data?.error || "Unable to load archived chat messages.");
        })
        .finally(() => {
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-slate-50/70">
          ...
          <main className="flex-1 p-8 space-y-6 max-w-7xl w-full mx-auto">
            ...
            <section className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs" data-purpose="sessions-table">
              <div
                className="max-h-[460px] overflow-x-auto overflow-y-scroll"
                aria-label="Scrollable session list"
              >
                <table className="w-full text-left text-xs">
                  <thead className="sticky top-0 z-10 bg-slate-50 border-b border-slate-200 text-slate-600 font-semibold uppercase tracking-wider text-[10px]">
                    <tr>
            ...
            <section className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm" data-purpose="session-detail-card">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-5 border-t lg:border-t-0 lg:border-l lg:border-r border-slate-200 lg:px-6 pt-4 lg:pt-0">
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50/90 p-4 text-xs text-slate-500 max-h-80 overflow-y-auto space-y-3">
                    {isLoadingMessages && <p>Loading archived chat messages…</p>}
                    {!isLoadingMessages && messageError && <p className="text-rose-600">{messageError}</p>}
                    {!isLoadingMessages && !messageError && !selectedSession.archivedAt && (
                      <p>This session has not been archived yet.</p>
                    )}
                    {!isLoadingMessages && !messageError && selectedSession.archivedAt && selectedSession.messages.length === 0 && (
                      <p>This archive contains no supported chat messages.</p>
                    )}
                    {!isLoadingMessages && !messageError && selectedSession.messages.map((message) => (
                      <article key={message.id} className="rounded border border-slate-200 bg-white p-2.5">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <span className="font-mono font-semibold text-slate-700">{message.sender}</span>
                          <time className="font-mono text-[10px] text-slate-400">{formatDateTime(message.time)}</time>
                        </div>
